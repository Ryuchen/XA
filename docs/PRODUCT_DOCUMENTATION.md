...
| 老板 | 微信小程序（不支持 H5） | `apps/boss-miniapp/` | 微信快捷登录、浏览与选择服务/陪玩、下单支付、查单评价和获取售后 |
...
- 钱包展示余额及资金流水。
- 签到为**消费型签到**：当日消费达门槛（默认 188 元等值）方可签到，按“本月第几次签到”的阶梯发放礼物 + 余额奖励；消费达更高门槛（默认 388 元等值）额外发补签卡（每月上限 3 张，可补签本月历史缺勤日）；当月签满自动发放全勤专属 Tag 并推送站内消息。规则由后台单例 `CheckinRuleConfig` 配置，详见《产品开发文档》§4.10。
